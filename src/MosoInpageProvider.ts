...
export default class MosoInpageProvider extends SafeEventEmitter {
   ...
   * If null, Moso is either locked or the user has not permitted any
   * addresses to be viewed.
   */
  public selectedAddress: string | null;

  /**
   * Indicating that this provider is a Moso provider.
   */
  public readonly isMoso: true;

  /**
   * Experimental methods can be found here.
   */
  public readonly _moso: ReturnType<MosoInpageProvider['_getExperimentalApi']>;

  /**
   * @param connectionStream - A Node.js duplex stream
   * @param options - An options bag
   * @param options.jsonRpcStreamName - The name of the internal JSON-RPC stream.
   * Default: moso-provider
  ...
  constructor(
    ...
    {
      jsonRpcStreamName = 'moso-provider',
      logger = console,
      maxEventListeners = 100,
      shouldSendMetadata = true,
    }: MosoInpageProviderOptions = {},
  ) {
    ...
    this._moso = this._getExperimentalApi();
    ...
    pump(
      ...
      connectionStream,
      this._handleStreamDisconnect.bind(this, 'Moso', new Error()),
    );
    ...
    pump(
      ...
      jsonRpcConnection.stream,
      this._handleStreamDisconnect.bind(this, 'Moso RpcProvider', new Error()),
    );
    ...
    jsonRpcConnection.events.on('notification', (payload) => {
      ...
      if (method === 'moso_accountsChanged') {
        this._handleAccountsChanged(params);

      } else if (method === 'moso_unlockStateChanged') {
        this._handleUnlockStateChanged(params);
      } else if (method === 'moso_chainChanged') {
      ...
      } else if (method === 'MOSO_STREAM_FAILURE') {
  ...
  /**
   * Constructor helper.
   * Populates initial state by calling 'moso_getProviderState' and emits
   * necessary events.
   */
  private async _initializeState() {
    try {
      ...
      this._log.error(
        'Moso: Failed to get initial state. Please report this bug.',
        error,
      );
    } finally {
   ...
   * @emits MosoInpageProvider#connect
   */
   ...
   * @emits MosoInpageProvider#disconnect
   */
   ...
   *
   * @emits MosoInpageProvider#chainChanged
  ...
  private _handleChainChanged({
    ...
    if (
      ...
      this._log.error(
        'Moso: Received invalid network parameters. Please report this bug.',
  ...
  private _handleAccountsChanged(accounts: unknown[], isEthAccounts = false): void {
    ...
    if (!Array.isArray(accounts)) {
      this._log.error(
        'Moso: Received invalid accounts parameter. Please report this bug.',
        accounts,
      );
      _accounts = [];
    }
    ...
    for (const account of accounts) {
      if (typeof account !== 'string') {
        this._log.error(
          'Moso: Received non-string account. Please report this bug.',
          accounts,
        );
        _accounts = [];
        break;
    ...
    if (!dequal(this._state.accounts, _accounts)) {
      ...
      if (isEthAccounts && this._state.accounts !== null) {
        this._log.error(
          `Moso: 'eth_accounts' unexpectedly updated accounts. Please report this bug.`,
          _accounts,
        );
      }
  ...
  private _handleUnlockStateChanged({
    ...
    if (typeof isUnlocked !== 'boolean') {
      this._log.error('Moso: Received invalid isUnlocked parameter. Please report this bug.');
      return;
    }
  ...
  /**
   * Constructor helper.
   * Gets experimental _moso API as Proxy, so that we can warn consumers
    ...
    return new Proxy(
      {
        ...
        /**
         * Determines if Moso is unlocked by the user.
         *
         * @returns Promise resolving to true if Moso is currently unlocked
         */
        isUnlocked: async () => {
          if (!this._state.initialized) {
