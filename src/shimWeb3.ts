import MosoInpageProvider from './MosoInpageProvider';
...
export default function shimWeb3(
  ...
  if (!(window as Record<string, any>).web3) {
    const SHIM_IDENTIFIER = '__isMosoShim__';
      ...
      web3Shim,
      {
        get: (target, property, ...args) => {
          if (property === 'currentProvider' && !loggedCurrentProvider) {
            ...
            provider.request({ method: 'moso_logWeb3ShimUsage' })
              .catch((error) => {
                log.debug('Moso: Failed to log web3 shim usage.', error);
              });
          }
